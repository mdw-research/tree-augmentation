...
def boxPlot(filename, size):
    ...
    exact_idx = data.columns.get_loc(" randomized")

    # Define the columns to be plotted (columns to the right of "randomized")
    columns_to_plot = data.columns[exact_idx:]

    # Create a figure to hold all subplots
    fig, axes = plt.subplots(nrows=2, ncols=3, figsize=(15, 10))
    fig.suptitle(f'Solution Sizes for Each Tree Type (n = {size})')

    # Boxplot style properties for thinner lines
    ...
    for row_idx, tree_type in enumerate(tree_types_capitalized):
        ...
        # Compute the ratios
        ratios = filtered_data[columns_to_plot]

        # Create box and whiskers plot for the ratios
        axes[row, col].boxplot(
            [ratios[column].dropna() for column in columns_to_plot], 
            positions=range(1, len(columns_to_plot) + 1),
            boxprops=boxprops,
        ...
        axes[row, col].set_ylabel('Solution Size')
...
if __name__ == '__main__':
    boxPlot("results20240613-071754.txt", 1000)
